...
class BST_Bad_Repository:
    ...
    def specification(self):
        ...
        return {
            "Node": SpecificationBuilder()
            .parameter("l", labels)
            .suffix(Constructor("BST") & Constructor("height", Literal(0)) & Constructor("label", Var("l"))),
            ...
            .parameter("h", heights, lambda vs: [max(vs["h1"], vs["h2"]) + 1])
            .parameter("l", labels)
            .argument("left", Constructor("BST") & Constructor("height", Var("h1")))
            .argument("right", Constructor("BST") & Constructor("height", Var("h2")))
            .constraint(lambda v: self.is_sorted(v["left"].interpret(self.inorder_traverse_algebra()) + [v["l"]] + v["right"].interpret(self.inorder_traverse_algebra())))
            .suffix(Constructor("BST") & Constructor("height", Var("h")))
        }
...
if __name__ == "__main__":
    ...
    repo_bad = BST_Bad_Repository(5,["A", "B", "C"])

    target_bad = Constructor("BST")
    ...
    print(f"The bad repository leads to a solution space with {len(bad_grammar)} rules, while the proposed good repository leads to a solution space with {len(good_grammar)} rules.")
